...
public class HSVColorWheel extends View {
	...
	private static final float FADE_OUT_FRACTION = 0.05f;
	
	private static final int POINTER_LINE_WIDTH_DP = 4;
	...
	protected void onDraw(Canvas canvas) {
		if ( bitmap != null ) {
            ...
            canvas.clipPath(innerCirclePath, Region.Op.INTERSECT);
	...
	protected void onSizeChanged(final int w, final int h, int oldw, int oldh) {
		...
		// run this on a background thread, otherwise the UI will lag like hell
		new Thread(new Runnable() {
		    public void run() {
                ...
                innerCirclePath.addCircle(rect.centerX(), rect.centerY(), innerCircleRadius-5, Path.Direction.CW);
	...
	private void createBitmap() {
		...
		// invalidate needs to be called from the UI thread
